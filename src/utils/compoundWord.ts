...
const MORPHEME_LEXICON: { thai: string; meaning: string }[] = [
  ...
  { thai: 'จำ', meaning: '記得' },
  { thai: 'ลืม', meaning: '忘記' },
  { thai: 'จดจำ', meaning: '銘記／記住' },
  { thai: 'ลง', meaning: '下／下降' },
  { thai: 'ลุก', meaning: '起身' },
  { thai: 'ห่อ', meaning: '包／打包' },
  { thai: 'ทัน', meaning: '來得及／跟得上' },
  { thai: 'ทันที', meaning: '立刻' },
...
function buildMeaningMap(): Map<string, string> {
  ...
  map.set('มา', '來')
  map.set('กิน', '吃')
  map.set('ด้วย', '與／一起（前接）')
  map.set('กัน', '互相／一起（助詞）')
  map.set('ด้วยกัน', '一起')
  return map
}
