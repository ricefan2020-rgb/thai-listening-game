...
export type FoodArea = HotelArea | 'any'
...
export const FOOD_AREA_LABELS: Record<FoodArea, string> = {
...
export const FOODS: FoodRecommendation[] = [
  {
...
export function getRecommendedFoods(
  ...
  return FOODS.map((f) => ({ f, score: scoreFood(f, config) }))
    .filter((x) => x.score >= 0)
