...
function pickPlaces(config: TripConfig): Place[] {
  ...
  const ranked = [...PLACES].sort((a, b) => scorePlace(b, config) - scorePlace(a, config))
  const picked: Place[] = []
...
export function normalizeConfig(config: TripConfig): TripConfig {
  ...
  return {
    ...
    currency,
    exchangeRate:
      config.exchangeRate > 0 ? config.exchangeRate : meta.defaultUnitsPerThb,
...
export function exportPlanMarkdown(plan: TripPlan): string {
  ...
  lines.push('', '## 住宿推介', '')
  lines.push(`- 區域：${HOTEL_AREA_LABELS[config.hotelArea]} · 約 ${nights} 晚`, '')
