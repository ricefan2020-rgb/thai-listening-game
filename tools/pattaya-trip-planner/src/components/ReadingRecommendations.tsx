...
export function ReadingRecommendations({ plan }: ReadingRecommendationsProps) {
  ...
  const recs = getRecommendationsForTrip(plan.config.interests, 6)

  return (
    <section className="panel-card rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
      <h3 className="text-lg font-bold text-slate-900">文章推薦</h3>
      <p className="mt-1 text-sm text-slate-600">依你的興趣自動推薦：交通、海灘、美食與泰文短文</p>
