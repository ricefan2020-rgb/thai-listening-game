# 泰國旅遊計劃（Thailand Travel Journal）

依天數、人數、預算與興趣自動產生泰國多地行程（芭提雅、曼谷、清邁、普吉、喀比）；以**部落格筆記**形式閱讀，支援**小紅書式發布**、**雲端廣場**與**複製他人行程**。

## 啟動

```bash
npm run thailand:dev
```

（`npm run pattaya:dev` 為同指令別名，相容舊習慣。）

瀏覽器開啟：`http://localhost:5173/tools/thailand-trip-planner/`
...
產出：`dist/tools/thailand-trip-planner/index.html`
...
| 模式 | 格式 |
|------|------|
| 雲端 | `.../thailand-trip-planner/?post=<uuid>` |
| 本機 | `.../thailand-trip-planner/?share=<gzip-base64>` |
