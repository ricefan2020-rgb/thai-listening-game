import type { Interest, ThailandRegionId } from '../types'
...
export function getRecommendationsForTrip(
  regionId: ThailandRegionId,
  interests: Interest[],
  limit = 6,
): ReadingRecommendation[] {
  const scored = READING_LIST.filter((item) => {
    if (!item.regions || item.regions.length === 0) return true
...
export function appArticleUrl(articleId: string): string {
  const base = import.meta.env.BASE_URL.replace(
    /\/tools\/(?:thailand|pattaya)-trip-planner\/?$/,
    '/',
  )
