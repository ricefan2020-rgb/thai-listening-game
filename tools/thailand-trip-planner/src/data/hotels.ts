...
import { REGION_HOTELS } from './hotels-regions'
...
function tagPattaya(hotels: HotelInput[]): HotelRecommendation[] {
  return hotels.map((h) => ({ ...h, regionId: 'pattaya' as ThailandRegionId }))
}

const PATTAYA_HOTELS_RAW: HotelInput[] = [
  // —— 市中心 ——
  {
    id: 'sawasdee-coco',
...
export const ALL_HOTELS: HotelRecommendation[] = [
  ...tagPattaya(PATTAYA_HOTELS_RAW),
  ...REGION_HOTELS,
]

/** @deprecated 使用 ALL_HOTELS */
export const HOTELS = ALL_HOTELS

function scoreHotel(hotel: HotelRecommendation, config: TripConfig): number {
  if (hotel.regionId !== config.regionId) return -1000
...
export function getRecommendedHotels(
  ...
  return ALL_HOTELS.filter(
    (h) => h.regionId === config.regionId && h.area === config.hotelArea,
  )
